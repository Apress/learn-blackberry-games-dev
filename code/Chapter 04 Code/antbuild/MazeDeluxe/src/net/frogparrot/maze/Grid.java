...
public class Grid {
  ...
  /**
   * @return whether the current square is filled
   * (part of the wall).
   * @param x the X-coordinate in the grid's coordinate
   *        system (not screen graphics coordinates).
   * @param y the Y-coordinate in the grid's coordinate
   *        system (not screen graphics coordinates).
   */
  public boolean isWall(int x, int y) {
    // anything outside the maze is not a wall:
    if((x < 0)
       || (y < 0)
       || (x >= mySquares.length)
       || (y >= mySquares[0].length)) {
      return false;
    }
    return mySquares[x][y] == 0;
  }
  ...
  Grid(int width, int height) {
    ...
    mySquares[0][1] = 1;
    // the exit is at (width - 1, height - 2)
    mySquares[width-1][height-2] = 1;
    createMaze();
  }
