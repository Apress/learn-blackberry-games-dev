...
public class PrefsStorage {
   ...
   */
  private static final String STORE = "SizePrefs";
   ...
   */
  public static int getSquareSizeIndex() {
    // if data retrieval fails, the default value is 1
    int retVal = 1;
   ...
   */
  public static void setSquareSizeIndex(int size) {
    RecordStore store = null;
    try {
      // since we are storing the int as a single byte, 
      // it is very important that it's value be less than 128.
      // This could be changed to serialize and store a whole int.
      if(size > 127) {
        size = 127;
      }
